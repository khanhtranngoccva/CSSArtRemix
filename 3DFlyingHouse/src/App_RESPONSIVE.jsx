...
import c1 from "./react-dimension-css/DimensionCSS.module.css";
import classes from "./house_RESPONSIVE.module.css";
...
function Balloon() {
    ...
    const w = React.useMemo(() => Math.round(Math.random() * 10 + 40) / 10, []);
    return <TransformGroup className={classes.balloonAnchor}>
        <Plane className={classes.balloon} style={{
            "--width": `calc(${w} * var(--unitSize)`,
            "--height": `calc(${1.25 * w} * var(--unitSize)`,
...
function String() {
    const height = React.useMemo(() => (Math.random() * 200 + 100) / 10, []);
    ...
    return <TransformGroup className={classes.stringAnchor} style={{
        "--stringRotZ": rz + "deg",
        "--stringRotX": rx + "deg",
        "--stringHeight": `calc(${height} * var(--unitSize))`,
    }}>
...
function App() {
    return <Canvas className={classes.canvas} minRotX={-30} maxRotX={30}>
        <House></House>
    </Canvas>
}
