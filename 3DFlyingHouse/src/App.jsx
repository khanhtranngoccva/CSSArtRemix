...
function Balloon() {
    ...
    const w = Math.round(Math.random() * 10 + 40);
...
function String() {
    ...
    const rzr = 60;
    const rxr = 60;
    const rz = (Math.random() * rzr - rzr / 2).toFixed(2);
    const rx = (Math.random() * rxr - rxr / 2).toFixed(2);
...
function House() {
    return <TransformGroup className={classes.house}>
...
function App() {
    return <Canvas allowRotate={true} className={classes.canvas} minRotX={-30} maxRotX={-30}>
        <House></House>
    </Canvas>
}
